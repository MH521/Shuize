...
def collation_web_host(Subdomains_ips, webSpace_web_host_port, ip2domainSubdomains):
    ...
    #         # if len(parts)>=3:
    #         #     web_host_port_temp.append(':'.join(parts[:3]))
    #         # else:
    #         web_host_port_temp.append(subdomain) 
    #     else:
    #         web_host_port_temp.append('{}:80'.format(subdomain))
    #         web_host_port_temp.append('{}:443'.format(subdomain))
    web_host_port_temp=list(set(list(Subdomains_ips.keys()) + ip2domainSubdomains))
    ...
    # for subdomain in web_host_port_temp_temp:
    #     if ':' in subdomain:
    #         parts = subdomain.split(':', 3)
    #         if len(parts)>=3:
    #             web_host_port_temp.append(':'.join(parts[:3]))
    ...
    for host_port in web_host_port_temp:
        ...
        if host_port_urlparse.scheme not in scheme:           # 如果没有http（https）， 则加上。如果是443端口，则加https，其他端口加http
            if ':' in host_port:
                try:
                    ...
                    if port == '443':
                        ...
                        web_host_port.append('http://{}'.format(host_port))
                        web_host_port.append('https://{}'.format(host_port))
                except Exception as e:
                    pass
            else:
                #host_port = 'http://{}'.format(host_port)
...
def run_cSubnet(CIP_List, Subdomains_ips, notCDNSubdomains, param_Links):
    ...
    SubdomainAndNotCDNIPs = []  # 子域名和非CDN、非云的IP
...
def banner():
  ...
  | () |  \ \ /     / /    / /      / /  
   ...
   脚本自带Linux版本的Nuclei、ksubdomain和xapp，如果是windows或者mac，需要自行更换版本。
   Plugins/infoGather/subdomain/ksubdomain/ksubdomain.py
   Plugins/Vul/Nuclei/NucleiApi.py
   Plugins/infoGather/webInfo/Xapp/xapp.py
